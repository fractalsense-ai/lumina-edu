...
| 2 | **Domain NLP Pre-Interpreter** | Domain pack | `model-packs/<domain>/controllers/nlp_pre_interpreter.py` | `evidence` dict + `_nlp_anchors` list |
...
- [`model-packs/education-admin/controllers/governance_adapters.py`](../../model-packs/education-admin/controllers/governance_adapters.py) - `_deterministic_command_fallback()` - domain-specific verb/noun routing for admin commands (invite, modify, deactivate)
