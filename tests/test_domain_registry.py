...
@pytest.mark.unit
def test_domain_authority_with_foundation_module_returns_foundation(registry: DomainRegistry) -> None:
    """admin with a domain/eduf/... module -> education-foundation domain."""
    user = {
        "sub": "da_eduf_001",
        "role": "admin",
        "governed_modules": ["domain/eduf/foundation-core/v1"],
    }
    assert registry.resolve_default_for_user(user) == "education-foundation"
...
@pytest.mark.unit
def test_domain_authority_with_math_module_returns_math(registry: DomainRegistry) -> None:
    """admin with a domain/edumath/... module -> education-math domain."""
    user = {
        "sub": "da_edumath_001",
        "role": "admin",
        "governed_modules": ["domain/edumath/pre-algebra/v1"],
    }
    assert registry.resolve_default_for_user(user) == "education-math"
...
@pytest.mark.unit
def test_edumath_prefix_resolves_to_education_math(registry: DomainRegistry) -> None:
...
def test_unauthenticated_domain_separate_from_default(tmp_path: Path) -> None:
    ...
    reg_data = {
        "unauthenticated_domain": "education-foundation",
        "default_domain": "education",
        "domains": {
            "education": {
                "runtime_config_path": "model-packs/education/cfg/runtime-config.yaml",
    ...
    assert reg.resolve_default_for_user(None) == "education-foundation"
