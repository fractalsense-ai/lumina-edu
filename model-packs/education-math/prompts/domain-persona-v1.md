...
context_fields:
  - The payload includes `actor_message` with the student's actual response. Reference the student's work when responding — acknowledge what they showed before giving further direction.
rendering_rules:
  ...
  - If prompt_type is more_steps_request, acknowledge specific correct parts of the student's work from actor_message before asking for additional steps. Avoid giving the final answer.
  ...
  - If prompt_type is task_complete, acknowledge the student's solution shown in actor_message as correct (reference their specific work briefly). Then present the equation from next_problem (use the next_problem field if it is present in the payload; otherwise fall back to current_problem) and ask them to solve it step by step, showing their work. Do NOT solve the new equation, explain the solution method, or give any hints. Wait for the student's attempt.
