...
- Renamed `learning_state.zpd_band` → `learning_state.challenge_band` in student profiles
  to align with the updated education domain `model-packs/education/schemas/compressed-state-schema-v1.json` and
  `model-packs/education/schemas/student-profile-schema-v1.json`, which now use the domain-neutral field name
