 ...
 *
 * Fetches the education-admin roster-status view and renders each teacher with
    ...
    <div className="flex flex-col gap-4">
      ...
      {teachers.map((teacher) => (
        <Card
          ...
          className={`p-4 border-l-4 ${COLOR_BORDER[teacher.color] ?? 'border-l-gray-400'}`}
        >
          <div className="flex items-center gap-2 mb-2">
            <span className={`inline-block w-2.5 h-2.5 rounded-full ${COLOR_CLASSES[teacher.color] ?? 'bg-gray-400'}`} />
            <span className="font-medium text-sm text-foreground">{teacher.username}</span>
          </div>

          <div className="grid grid-cols-3 gap-2 text-center">
            <div>
              <p className="text-lg font-semibold text-foreground">{teacher.student_count}</p>
              <p className="text-xs text-muted-foreground">Students</p>
            </div>
            <div>
              <p className={`text-lg font-semibold ${teacher.pending_escalations > 0 ? 'text-amber-500' : 'text-foreground'}`}>
                {teacher.pending_escalations}
              </p>
              <p className="text-xs text-muted-foreground">Pending</p>
            </div>
            <div>
              {teacher.has_sla_breach ? (
          ...
          {teacher.modules.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {teacher.modules.map((moduleId) => (
                <span key={moduleId} className="text-xs px-1.5 py-0.5 rounded bg-muted text-muted-foreground">
